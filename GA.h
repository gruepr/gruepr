...
namespace GA
{
    ...
    inline static const int POPULATIONSIZE = 30000;			// the number of genomes in each generation--larger size is slower, but each generation is more likely to have optimal result.
    ...
    inline static const int TOURNAMENTSIZE = POPULATIONSIZE/500;    // most of the next generation is created by mating many pairs of parent genomes, each time chosen from genomes in a randomly selected tournament in the genepool
    ...
    inline static const int GENOMESIZETHRESHOLD[] = {25, 60};       // threshold values of genome size that decide which TOPGENOMELIKELIHOOD value to use -- if genome is <= threshold 1, use smallest likelihood; if <= threshold 2, use medium likelihood
