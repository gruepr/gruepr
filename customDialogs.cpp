...
/*
/////////////////////////////////////////////////////////////////////////////////////////////////////////
// A dialog window to add or remove students from the current working "database"
/////////////////////////////////////////////////////////////////////////////////////////////////////////

addRemoveStudentsDialog::addRemoveStudentsDialog(const studentRecord student[], const int totalNumStudents, const bool genderIncluded, QWidget *parent)
    : QDialog(parent)
{
    //copy data into local versions, including full database of students
    numStudents = totalNumStudents;
    this->student = new studentRecord[numStudents+100];   //getting enough room to add 100 students
    for(int i = 0; i < numStudents; i++)
    {
        this->student[i] = student[i];
    }
    ...
    //Set up window
    setWindowTitle(tr("Add or Remove Students"));
    setWindowFlags(Qt::Dialog | Qt::WindowTitleHint | Qt::WindowCloseButtonHint);
    setSizeGripEnabled(true);
    setMinimumSize(600, 600);
    theGrid = new QGridLayout(this);
    ...
    //First row - the current data
    currentListOfStudentsTable = new QTableWidget(this);
    currentListOfStudentsTable->setSortingEnabled(true);
    currentListOfStudentsTable->setEditTriggers(QAbstractItemView::NoEditTriggers);
    currentListOfStudentsTable->setAlternatingRowColors(true);
    currentListOfStudentsTable->setStyleSheet(
            "QTableView{gridline-color: black;}"
            "QHeaderView::section{"
                "border-top:0px solid #D8D8D8;"
                "border-left:0px solid #D8D8D8;"
            ...
            "background-color:white;"
            "}");
    currentListOfStudentsTable->setColumnCount(4);
    currentListOfStudentsTable->setHorizontalHeaderItem(0, new QTableWidgetItem(tr("First Name")));
    currentListOfStudentsTable->setHorizontalHeaderItem(1, new QTableWidgetItem(tr("Last Name")));
    currentListOfStudentsTable->setHorizontalHeaderItem(2, new QTableWidgetItem(tr("Section")));
    currentListOfStudentsTable->setHorizontalHeaderItem(3, new QTableWidgetItem("  "));
    theGrid->addWidget(currentListOfStudentsTable, 0, 0, 1, -1);

    //Second row - add a student boxes and buttons
    firstname.setPlaceholderText(tr("First Name"));
    theGrid->addWidget(&firstname, 2, 0);
    lastname.setPlaceholderText(tr("Last Name"));
    theGrid->addWidget(&lastname, 2, 1);
    email.setPlaceholderText(tr("email"));
    theGrid->addWidget(&email, 2, 2);
    if(genderIncluded)
    {
        gender.addItem(tr("woman"));
        gender.addItem(tr("man"));
        gender.addItem(tr("non-binary"));
        theGrid->addWidget(&gender, 2, 3);
    }
    ...
    {
        sectionNames.sort();
        section.insertItems(0,sectionNames);
        theGrid->addWidget(&section, 2, 4);
    }
    addStudent = new QPushButton(this);
    addStudent->setText(tr("&Add student"));
    connect(addStudent, &QPushButton::clicked, this, &addRemoveStudentsDialog::addAStudent);
    theGrid->addWidget(addStudent, 2, 5);
...
addRemoveStudentsDialog::~addRemoveStudentsDialog()
{
    //delete dynamically allocated arrays created in class constructor
    delete [] student;
}
...
void addRemoveStudentsDialog::removeAStudent()
{
    //Search through all the students and, once we found the one with the matching ID, move all remaining ones up by one and decrement numStudents
    bool foundIt = false;
    for(int i = 0; i < numStudents; i++)
    {
...
void addRemoveStudentsDialog::addAStudent()
{
    student[numStudents].firstname = firstname.text();
    student[numStudents].firstname[0] = student[numStudents].firstname[0].toUpper();
    student[numStudents].lastname = lastname.text();
    student[numStudents].lastname[0] = student[numStudents].lastname[0].toUpper();
    student[numStudents].email = email.text();
    student[numStudents].section = section.currentText();
    student[numStudents].woman = (gender.currentText()==tr("woman"));
    student[numStudents].ID = numStudents + 1000;   //flag for added students is an ID > 1000
    for(int i = 0; i < maxAttributes; i++)
    {
        student[numStudents].attribute[i] = -1;     //set all attribute levels to -1 as a flag to ignore during the teaming
    }
    for(int time = 0; time < numTimeBlocks; time++)
    {
        student[numStudents].unavailable[time] = false;
    }
    numStudents++;
    refreshDisplay();
}
...
void addRemoveStudentsDialog::refreshDisplay()
{
    currentListOfStudentsTable->clearContents();
    currentListOfStudentsTable->setSortingEnabled(false);    //need to temporarily disable sorting while adding items
    currentListOfStudentsTable->setRowCount(numStudents);
    for(int i = 0; i < numStudents; i++)
    {
        currentListOfStudentsTable->setItem(i, 0, new QTableWidgetItem(student[i].firstname));
        currentListOfStudentsTable->setItem(i, 1, new QTableWidgetItem(student[i].lastname));
        currentListOfStudentsTable->setItem(i, 2, new QTableWidgetItem(student[i].section));
        QPushButton *remover = new QPushButton(tr("Remove student"), this);
        remover->setProperty("StudentID", student[i].ID);
        connect(remover, &QPushButton::clicked, this, &addRemoveStudentsDialog::removeAStudent);
        currentListOfStudentsTable->setCellWidget(i, 3, remover);
    }
    currentListOfStudentsTable->resizeColumnsToContents();
    currentListOfStudentsTable->setSortingEnabled(true);
}
*/
