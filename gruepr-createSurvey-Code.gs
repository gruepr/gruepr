...
function doGet(e) {
  ...
  var includeGender = false;
  if('gend' in dataAll) {
    includeGender = (dataAll["gend"]=="true");
  }
  var genderType = 1;
  if('gentype' in dataAll) {
     genderType = parseInt(dataAll["gentype"],10);
  }
  ...
  var attributeMulti = ["false", "false", "false", "false", "false", "false", "false", "false", "false", "false", "false", "false", "false", "false", "false"];
  if('attrmulti' in dataAll) {
    attributeMulti = (dataAll["attrmulti"]).split(",");
  }
  ...
  var includePrefTeammates = false;
  if('prefmate' in dataAll) {
    includePrefTeammates = (dataAll["prefmate"]=="true");
  }
  var includePrefNonTeammates = false;
  if('prefnon' in dataAll) {
    includePrefNonTeammates = (dataAll["prefnon"]=="true");
  }
  var numPrefMates = 1;
  ...
  if(includeGender) {
    var genderQuestion = 'With which gender do you identify most closely?';
    var choices=[];
    if(genderType == 0) {
      choices = ['Female','Male','Nonbinary','Prefer Not to Answer'];
    } else if(genderType == 1) {
      choices = ['Woman','Man','Nonbinary','Prefer Not to Answer'];
    } else if(genderType == 2) {
      choices = ['Girl','Boy','Nonbinary','Prefer Not to Answer'];
    } else if(genderType == 3) {
      genderQuestion = 'What are your pronouns?';
      choices = ['She-her','He-him','They-them','Prefer Not to Answer'];
    }
    form.addListItem()
      .setTitle(genderQuestion)
